% Aufgabe war es, die Grossmutter, Geschwister, Bruder und Tante zu definieren.
% Die Grossmutter kann gefunden werden, indem die Mutter der Mutter gefunden wird.
% Geschwister koennen gefunden werden, indem die Kinder der gleichen Eltern gefunden werden, die nicht die gleiche Person sind.
% Bruder kann gefunden werden, indem maennliche Geschwister der Person gefunden werden.
% Tante kann gefunden werden, indem nach weiblichen Geschwistern der Mutter der Person gesucht wird.
...
% Aunt can be found by searching for female siblings of person's mother
tante(Person, Tante) :-
    mutter(Person, Mutter),
